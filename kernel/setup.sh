...
# Linux LTSI
ltsi_branch="master"
ltsi_srcrev="5a62a61cd5f0ebd29a85f4751c6856ca07e6d713"
...
clone_linux_ltsi () {
	cur_dir=$(pwd)
	LTSI_GIT_REPO="http://git.linuxfoundation.org/ltsi-kernel.git"
	if [ ! -d "repo-ext/linux-ltsi" ]
	then
		echo "Creating new linux-ltsi repo"
		mkdir -p repo-ext/linux-ltsi
		cd repo-ext/linux-ltsi
		git init
		git remote add -t ${ltsi_branch} origin ${LTSI_GIT_REPO}
		git fetch origin
		git checkout -b ${ltsi_branch} --track origin/${ltsi_branch}
	else
		cd repo-ext/linux-ltsi
		echo "Updating linux-ltsi repo"
		git fetch origin
	fi
	cd ${cur_dir}
}

create_linux_lts_ltsi () {
	cur_dir=$(pwd)
	cd ${cur_dir}/repo-ext/linux-stable
	git checkout linux-${KVER}.y
	git reset --hard HEAD
	cd ${cur_dir}/repo-ext/linux-ltsi
	git checkout master
	git reset --hard ${ltsi_srcrev}
	git archive ${ltsi_branch} | tar -xC ${cur_dir}/repo-ext/linux-stable
	cd ${cur_dir}/repo-ext/linux-stable
	ltsi_ws_branch=`git branch | grep "${KVER}.*-ltsi" | cut -c2- | sed -e "s#\s*##g"`
	if [ -z "${is_branch}"  ]; then
		git branch -D ${ltsi_ws_branch}
	fi
	echo "Generating LTS & LTSI repo for Quark"
...
apply_linux_kernel_patch () {
	cur_dir=$(pwd)
	cd repo-ext/linux-stable
	BASE_BRC_ON=$(git branch | grep "${KVER}.*-ltsi" | grep "\*" | wc -l)
	if [ $BASE_BRC_ON -eq 1 ]; then
		echo "Linux LTSI ${KVER}.*-ltsi branch ready for patching ..."
		git am $cur_dir/intel-patchset/*.patch
	fi
	LOCAL_LINUX_BRANCH=$(git branch | grep "${KVER}.*-ltsi" | cut -c2- | sed -e "s#\s*##g")
	cd ${cur_dir}
}
